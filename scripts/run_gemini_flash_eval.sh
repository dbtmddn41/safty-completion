#!/usr/bin/env bash
# Evaluate gemini-3.1-flash-lite-preview on all three datasets.
...
run_eval() {
  ...
  echo ""
  echo "========================================"
  echo " gemini-flash-lite | $dataset_name"
  echo " input : $in_file"
...
run_eval "isolated_kept" \
  ...
  "$DATA/eval_gemini_flash_lite_isolated_kept.jsonl"

run_eval "isolated_nc6" \
  "$DATA/stage3_1_isolated_no_check6.jsonl" \
  "$DATA/eval_gemini_flash_lite_isolated_nc6.jsonl"

run_eval "audit_t4096" \
  "$DATA/stage3_1_audit_t4096_new.jsonl" \
  "$DATA/eval_gemini_flash_lite_audit_t4096.jsonl"

echo ""
echo "=== gemini-flash-lite: ALL DONE ==="
